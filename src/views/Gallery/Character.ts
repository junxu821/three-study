...
import { ON_KEY_DOWN } from "./constant";
import { MeshBVH } from "three-mesh-bvh";
import { type Emitter,  emitter } from "./Emitter";
...
export default class Character {
	...
	update(delta_time: number, collider: Mesh,boundsTree?:MeshBVH) {
		...
		// 调整相机视角
		this.core.camera.position.sub(this.core.orbitControls.target);
		this.core.orbitControls.target.copy(this.character.position);
	...
	* */
	...
	private _reset() {
		...
		this.core.orbitControls.target.copy(this.character.position);
		this.core.camera.position.add(this.character.position);
		this.core.orbitControls.update();
	}
	...
	private _updateCharacter(delta_time: number) {
		...
		const angle = this.core.orbitControls.getAzimuthalAngle();

		if (this.core.controlManage.mode === "pc") { // 根据PC端操作移动角色方位
			if (this.core.controlManage.key_status["KeyW"]) {
			...
			if (this.core.controlManage.key_status["KeyS"]) {
				this.temp_vector.set(0, 0, 1).applyAxisAngle(this.up_vector, angle);
				this.character.position.addScaledVector(this.temp_vector, this.speed * delta_time);
			}

			if (this.core.controlManage.key_status["KeyA"]) {
				this.temp_vector.set(-1, 0, 0).applyAxisAngle(this.up_vector, angle);
				this.character.position.addScaledVector(this.temp_vector, this.speed * delta_time);
			}

			if (this.core.controlManage.key_status["KeyD"]) {
				this.temp_vector.set(1, 0, 0).applyAxisAngle(this.up_vector, angle);
				this.character.position.addScaledVector(this.temp_vector, this.speed * delta_time);
			}
		} else { // 根据移动端操作移动角色方位
			const degree = this.core.controlManage.move_degree;
			if (degree) {
	...
	private _updateOrbitControls() {
		this.core.orbitControls.maxPolarAngle = Math.PI;
		this.core.orbitControls.minDistance = 1e-4;
		this.core.orbitControls.maxDistance = 1e-4;
	}
