import { ON_ENTER_APP } from "./constant";
...
export default class UI {
	...
	onClickEnterApp() {
		...
		this.emitter.emit(ON_ENTER_APP);
	}
	...
	updateLoadingProgress(loading_text: string) {
		const progress = this.doms.loading.querySelector(".progress") as HTMLElement;
        if(progress){
            progress.textContent = loading_text
        }
	}
